    ...
    for s in sorted(sub.sections, key=lambda x: x.order):
      ...
      <section class="sec" id="{anchor}">
        <button class="sec-head" type="button" aria-expanded="true">
          ...
          <div class="sec-h2row"><h2>{_esc(s.title)}</h2><span class="chev" aria-hidden="true">▾</span></div>
        </button>
...
<style>
  ...
  *{{box-sizing:border-box}} html{{scroll-behavior:smooth}}
  ...
  .sec-head{{display:block;width:100%;background:none;border:none;padding:0;margin:0;cursor:pointer;text-align:left;color:inherit;font:inherit}}
  .sec-h2row{{display:flex;align-items:flex-end;gap:14px}}
  .sec h2{{font-size:clamp(26px,3.4vw,38px);line-height:1.08;flex:1;margin:0 0 18px;border-bottom:2px solid var(--ink);padding-bottom:12px}}
  .chev{{font-family:var(--body);font-size:20px;line-height:1;color:var(--accent);margin-bottom:14px;transition:transform .2s}}
  .sec.collapsed .prose{{display:none}}
  .sec.collapsed .chev{{transform:rotate(-90deg)}}
  .sec.collapsed h2{{margin-bottom:0;border-bottom-color:var(--pale)}}
  .toc-ctl{{display:flex;gap:10px;margin:0 0 14px}}
  .toc-ctl button{{font-family:var(--mono);font-size:10px;letter-spacing:.08em;text-transform:uppercase;background:none;border:1px solid var(--pale);color:var(--mid);border-radius:4px;padding:5px 9px;cursor:pointer}}
  .toc-ctl button:hover{{border-color:var(--accent);color:var(--accent)}}
...
<body>
  ...
  <div class="shell">
    <nav class="toc"><p class="h">Contents</p>
      <div class="toc-ctl"><button id="expandAll">⊞ Expand</button><button id="collapseAll">⊟ Collapse</button></div>
      {toc}</nav>
...
<script>
  ...
  document.querySelectorAll('.sec-head').forEach(h=>h.addEventListener('click',()=>{{const s=h.closest('.sec');const c=s.classList.toggle('collapsed');h.setAttribute('aria-expanded',String(!c));}}));
  document.getElementById('collapseAll')?.addEventListener('click',()=>document.querySelectorAll('.sec').forEach(s=>{{s.classList.add('collapsed');s.querySelector('.sec-head')?.setAttribute('aria-expanded','false');}}));
  document.getElementById('expandAll')?.addEventListener('click',()=>document.querySelectorAll('.sec').forEach(s=>{{s.classList.remove('collapsed');s.querySelector('.sec-head')?.setAttribute('aria-expanded','true');}}));
